...
NON_RENTAL_PATTERNS = [
    ...
    (r"\b(massage|treatment|therapy|reiki|healing|wellness)\s+(room|space|studio\s+space|studio|practice)\s+(rental|for\s+rent|available|lease)\b",
                                                                  "wellness space"),
    # Office: "office space/suite for rent" OR bare "office for lease/rent"
    (r"\boffice\s+(space|suite)\b",                               "office space"),
    (r"\boffice\s+(for\s+rent|for\s+lease)\b",                    "office space"),
    (r"\bprofessional\s+(office|suite)\b",                        "professional office"),
    # Generic "suite for rent/lease" (FB commercial booth/office spam)
    (r"\bsuite\s+(for\s+rent|for\s+lease)\b",                     "commercial suite"),
    (r"\bprivate\s+office\b",                                     "private office"),
    (r"\b(commercial|retail|industrial)\s+(space|property|location|unit|rental|for\s+lease)\b",
                                                                  "commercial space"),
    (r"\bstorefront\b",                                           "storefront"),
    (r"\bco-?\s?working\b",                                       "coworking"),
    (r"\bevent\s+(space|venue|hall|center)\b",                    "event space"),
    (r"\b(banquet|party)\s+(hall|room|space)\b",                  "event space"),
    (r"\b(chair|booth)\s+rent(al)?\b",                            "chair/booth rental"),
    (r"\b(self[-\s]?storage|storage\s+unit|storage\s+of)\b",      "storage"),
    (r"\bfood\s+truck\s+(lot|space|spot)\b",                      "food truck lot"),
    (r"\b(boat|camper|rv|vehicle)\s+(storage|parking)\b",         "vehicle storage"),
    (r"\bshop\s+for\s+rent\b",                                    "commercial shop"),
    (r"\b(yoga|fitness|pilates|dance|workout)\s+studio\s+(for\s+rent|space\s+available|rental)\b",
                                                                  "fitness studio"),
    (r"\b(gym|warehouse|workshop)\s+(space\s+)?(for\s+rent|for\s+lease|available)\b",
                                                                  "commercial space"),
]
